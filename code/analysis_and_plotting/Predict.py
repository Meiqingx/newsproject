...
class Predict:
    ...
    def model(series, autoregressive_terms, num_years_to_predict = 1, 
        ...
        Inputs:
            ...
            autoregressive_terms = number of autoregressive terms.
              the number of variables to consider from the past
        '''
        # Params
        differences = 1
        moving_avg_terms = 0
    ...
    def predictions(model, series, num_years_to_predict = 1, 
    		independent_vars = None):
        '''
        ...
        if not isinstance(num_years_to_predict, int):
            ...
            print("Please use integers in number of years to predict.")
            print(" This time, we will predict", num_years_to_predict, "years")
        ...
        if independent_vars is not None:
            independent_vars = np.log(independent_vars)
            independent_future = SeriesRVO.series_for_prediction(independent_vars, 
        ...
        if independent_future is not None:
            predictions = results_ARIMA.predict(start=1,  end = len(
            	independent_future), exog = independent_future)[:-1]
        else:
            predictions = results_ARIMA.predict(start=1,  end = len(
            	dependent_future))[:-1]
        ...
        predictions_logs = predictions_logs.add(predictions_cumsum,
        	fill_value=0)
    ...
    def residuals(model, series, independent_vars = None):
        ...
        '''
        y_hat = Predict.predictions(model, series, 
        	independent_vars = independent_vars)[0]
    ...
    def durbin_watson(model, series, independent_vars = None):
        ...
        http://statsmodels.sourceforge.net/devel/_modules/statsmodels/stats/stattools.html
        ...
        '''
        resids = np.asarray(Predict.residuals(model, series, 
        	independent_vars))
    ...
    def adjusted_r_square(model, series, autoregressive_terms, 
    		independent_vars = None):
        '''
        ...
        rv = 1 - ((1 - r2) * ((sample_size - 1)/(
        	sample_size - number_predictors -1)))
    ...
    def best_order(series, independent_vars = None, stop_num_lags = 1):
        ...
        for i in list(range(1, stop_num_lags + 1)):
            model = Predict.model(series, i, 
            	independent_vars = independent_vars)[0]
            adjusted_r_square = Predict.adjusted_r_square(model, series, 
            	i, independent_vars)
            measures_of_fit_acumulator.put((-Predict.adjusted_r_square(
            	model, series, i, independent_vars), i))


        winner = measures_of_fit_acumulator.get()[1]
        return winner

    def best_parameters(name_column, database_dependent, 
    		database_independent, num_ind_var = 2):
        '''
        ...
        Outputs:
            ...
            list_independent_vars = list of the names of the ind. variables
            autoregressive_terms = best number of autoregressive terms
        ...
        for j in range(num_ind_var):
            ...
            for element in list_predictors:
                if element not in list_independent_vars:
                    ...
                    print("The independent vars are", list_independent_vars, 
                    	".Number regressive terms", autoregressive_terms)
                    independent_vars = database_independent[list_independent_vars]
                    model = Predict.model(series, autoregressive_terms, 
                    	independent_vars = independent_vars)[0]
                    adjusted_r_square = Predict.adjusted_r_square(model, 
                    	series, autoregressive_terms, independent_vars)
    ...
    def best_model(name_column, database_dependent, database_independent):
        ...
        variables, order = Predict.best_parameters(name_column, 
        	database_dependent, database_independent)

        series = database_dependent[name_column]
        independent_vars = database_independent[variables]

        best_model, differences = Predict.model(series, order, 
        	independent_vars = independent_vars)

        r2 = Predict.adjusted_r_square(best_model, series, order, 
        	independent_vars = independent_vars)
