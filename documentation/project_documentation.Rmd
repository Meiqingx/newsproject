...
This program predicts the prices of ten commodities based on a pool of 192 independent predictors:  64 first—order variables and their squared and cubic terms. Runing the program produces one pdf report for each of the ten commodities. 

> **How to run the program**        
  * Make sure all packages mentioned in the next section are properly installed   
  * run **getdata.sh** to scrape all the data, merge and process data, produce two csv files (one for independent variables, one for dependent variables), and N/50 diagnostic plotting graphs. (N is the number of independent variables.)   
  * run **run_files.py** to produce ten pdf reports of data analysis results, saved in a folder './reports'
...
* Description: 
A shell script that runs through all data scraping modules, the data merging module, and diagonistic plotting module. It produces two csv data files, and N/50 graph files. 
...
* Description:
    By default download 11 commodities by default (including crude oil, which is used as a predictor in our model)
...
# Reporting module
This module outputs a pdf report for each of the ten futures prices models. The ten reports are saved under './reports' folder.  
...
```
* Description: Take a dataframe of predition data and a dictionary of model analysis results, and build a report. 
